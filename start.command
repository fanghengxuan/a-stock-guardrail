#!/bin/bash
# ============================================================
# Guardrail 一键启动（macOS 访达双击）
# 后端先起 → 就绪后写 frontend/.env(VITE_USE_MOCK=false) → 前端起 → 打开浏览器
# Ctrl+C 同时关闭前后端
# ============================================================
set -uo pipefail
cd "$(dirname "$0")"
...
# P0-1: 后端可达 → 前端用真实模式（mock opt-in）
echo "VITE_USE_MOCK=false" > frontend/.env
green "✅ 已写 frontend/.env (VITE_USE_MOCK=false) — 前端连接真实后端"
