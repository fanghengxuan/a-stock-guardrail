"""分析报告生成模块：raw 数据 + 决策卡 → 三层 Markdown 分析报告（对齐「标的分析结果参考」）。
...
【参考范例 —— 模仿其表格样式、框线样式、"> 关键信号" 点评口吻与叙事密度；其章节为旧版 20 节结构，仅作风格参照，**章节结构一律以本规范 26 节为准**（新增节按本规范补写），数据一律替换为本标的真实数据】
"""
